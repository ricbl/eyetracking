...
def scroll(get_frame, t,table_et, table_transcription):
    ...
    xscreensize = (round(fixations['xmax_in_screen_coordinates'].values[0]*scale_video)-round(fixations['xmin_in_screen_coordinates'].values[0]*scale_video))
    ...
    # print(shown_part_frame.shape)
    image_resized = resize(shown_part_frame, (yscreensize,xscreensize),
                       anti_aliasing=True)*255
    # print(image_resized.max())
    # print(image_resized.min())
    # print(image_resized.shape)
    frame = np.zeros([round(2160*scale_video),round((3840-2*660)*scale_video),3]).astype(np.uint8)
    # print(frame.shape)
    # print(fixations['ymin_in_screen_coordinates'].values[0])
    # print(round((fixations['xmin_in_screen_coordinates'].values[0]-660)*scale_video),round((fixations['xmax_in_screen_coordinates'].values[0]-660)*scale_video),round((fixations['ymin_in_screen_coordinates'].values[0])*scale_video),round((fixations['ymax_in_screen_coordinates'].values[0])*scale_video))
