# generates a table summarizing some of the data collected for phase 2 from all the individual folders of data collected in phase 2
...
def get_classes_from_csv(csv_file, results_csv, user, transcriptions_csv):
    ...
    for trial in all_trials:
        ...
        if len(data_table_this_trial[data_table_this_trial['title']=='skip_image']['value'].values)>0:
            ...
            eng.append("generate_image_from_saved_data('../"+filepath+"', {["+' '.join([str(coord_value) for coord_value in coords_chest])+"]}, 0, 0, 0, 0,'./results_phase_2/"+user+"/chest_box_screen_"+str(trial)+".png',0,'"+user+"',0)")
            # eng.generate_image_from_saved_data(filepath, [coords_chest], 0, 0, 0, 0,'./results_phase_2/'+user+/'chest_box_screen_'+str(trial)+'.png',0, nargout=0)
            ...
            if labels_trial['Other']==-1:
                ...
                print(data_table_other_this_trial[data_table_other_this_trial['title']=='text']['value'].values)
            ...
            eng.append("generate_image_from_saved_data('../"+filepath+"', {"+','.join([('['+' '.join([str(coord_value) for coord_value in coords_this_rect])+']') for coords_this_rect in rect_list])+"}, 1, {'"+"','".join(labels)+"'},[" + \
                ';'.join(['['+' '.join([str(label_matrix[ni,nj]) for nj in range(label_matrix.shape[1])])+']'  for ni in range(label_matrix.shape[0])]) +'],'+ str(from_center)+",'./results_phase_2/"+user+"/bbox_screen_"+str(trial)+".png',1,'"+user+"',["+ \
                ' '.join([str((labels_trial[label_1]!=0)*1) for label_1 in labels])+"])")
            # eng.generate_image_from_saved_data(filepath, rect_list, 1, labels, label_matrix, from_center,'./results_phase_2/'+user+'/bbox_screen_'+str(trial)+'.png',1, nargout=0)
...
def analyze_interrater_reliability():
    data_folders = ['phase_2/user4_204_20210303-133107-9152',
    'phase_2/user1_204_20210301-141046-2142',
    'phase_2/user1_204_20210301-143153-9220',
    'phase_2/user5_204_20210302-142915-8332', 
    ...
    user_lists = []
    # print(eng)
    # result_file = open("lsit_of_matlabs_command_phase_2.csv",'w')
    ...
    use_trials = np.ones(full_array.shape[1], dtype=bool)
    ...
    results_csv.to_csv('results_phase_2.csv', index=False)
