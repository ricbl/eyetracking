...
class Extractor(object):
    ...
    def __init__(self, mention_phrases_dir, unmention_phrases_dir,
        ...
        self.observation2unmention_phrases\
            = self.load_unmentions(unmention_phrases_dir, "unmention")

    def load_unmentions(self, phrases_dir, phrases_type):
        """Read in map from observations to phrases for matching."""
        observation2phrases = defaultdict(list)
        observation2orders = defaultdict(lambda: defaultdict(list))
        for phrases_path in phrases_dir.glob("*.txt"):
            with phrases_path.open() as f:
                for line in f:
                    if line:
                        phrase = line.strip().replace("_", " ")
                        split_phrases_path = phrases_path.stem.split('}')
                        observation = split_phrases_path[0].replace("_", " ").lower()
                        if len(split_phrases_path)>1:
                            ordering = int(split_phrases_path[-1])
                            observation2orders[observation][ordering].append(phrase)
                        else:
                            observation2phrases[observation].append(phrase)
        for observation in observation2orders.keys():
            print(observation)
            a = []
            for ordering in range(len(observation2orders[observation].keys())):
                a.append(observation2orders[observation][ordering+1])
            for element in list(itertools.product(*a)):
                observation2phrases[observation].append("".join(element) )
        if self.verbose:
    ...
    def load_phrases(self, phrases_dir, phrases_type):
        ...
        for phrases_path in phrases_dir.glob("*.txt"):
            with phrases_path.open() as f:
                for line in f:
                    phrase = line.strip().replace("_", " ")
                    observation = phrases_path.stem.replace("_", " ").lower()
                    if line:
                        observation2phrases[observation].append(phrase)
